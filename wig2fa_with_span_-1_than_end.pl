...
#				warn "on pos $pos, value $val: IF\n";
				for (my $i = 0; $i < $pos; $i++) {
					print $out "N";
				}
			}
			elsif ($pos != $lastpos + $span + 1) {
#				warn "on pos $pos, value $val: ELSIF 1\n";
				for (my $i = $lastpos; $i < $pos; $i++) {
					print $out "N";
				}
			}
			else {
#				warn "on pos $pos, value $val: ELSIF 2: num is $number\n";
				for (my $i = $pos; $i < $pos+$span+1; $i++) {
					print $out "$number";
				}
			}
			$lastpos = $pos;
